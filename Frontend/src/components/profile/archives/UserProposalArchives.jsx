...
const UserProposalArchives = () => {
  ...
  const fetchUserProposals = async () => {
    try {
      console.log('📡 Fetching proposals for user:', currentUser.id);
      setLoading(true);
      setError(null);

      const response = await axios.get(
        `http://localhost:8000/api/user/proposals/${currentUser.id}`,
      ...
      if (response.data.success) {
        ...
        const cleanedProposals = (response.data.proposals || []).map((proposal, index) => ({
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Your proposal archives - Found {proposals.length} proposals
        </p>
      </div>
      ...
      <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4">
        <div className="flex items-center">
          ...
          <p className="text-sm text-green-700">
            ✅ Successfully loaded your proposals
          </p>
        </div>
      </div>
      ...
      {proposals.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-8 text-center">
          ...
          <p className="text-gray-600">
            You haven't created any proposals yet. Start by creating a new proposal!
          </p>
        </div>
      ) : (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="p-4 bg-gray-50 border-b">
            <h3 className="text-lg font-medium text-gray-900">Your Proposals ({proposals.length})</h3>
          </div>
...
export default UserProposalArchives; 